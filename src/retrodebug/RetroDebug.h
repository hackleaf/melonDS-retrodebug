...
private:
    ...
    // --- Bloom filter for breakpoint address fast-rejection ---
    // 4096 buckets, ref-counted. A zero bucket means no breakpoint hashes to
    // that slot, so we can skip the expensive subscription scan.
    // When stepping is active we bypass the bloom entirely (every PC fires).
    static constexpr unsigned BloomShift = 1; // ARM insns are >= 2-byte aligned
    static constexpr unsigned BloomBits  = 12;
    static constexpr unsigned BloomSize  = 1u << BloomBits;
    static constexpr unsigned BloomMask  = BloomSize - 1;

    // Two separate blooms: one per CPU (ARM9 / ARM7 breakpoints may differ)
    uint8_t BloomARM9_[BloomSize] = {};
    uint8_t BloomARM7_[BloomSize] = {};

    static unsigned BloomHash(uint32_t addr) {
        addr >>= BloomShift;
        return (addr ^ (addr >> BloomBits)) & BloomMask;
    }

    // Fast-path flags
    bool HasARM9BpSub_ = false;   // breakpoint-only subscriptions for ARM9
    bool HasARM9StepSub_ = false; // step subscriptions for ARM9 (bypass bloom)
    bool HasARM7BpSub_ = false;
    bool HasARM7StepSub_ = false;
    bool HasWriteSub_ = false;
    bool HasReadSub_ = false;
    bool HasDMASub_ = false;
    bool HasIPCSub_ = false;

    // Install/remove hook function pointers on the ARM CPU objects.
    // Called from RecomputeSubState() whenever subscription set changes.
    void UpdateHooks();

    // Static hook trampolines (called from ARM execute loop via function pointer)
    static bool ARM9HookTrampoline(u32 pc, void* userdata);
    static bool ARM7HookTrampoline(u32 pc, void* userdata);

    // The actual per-instruction check (called from trampoline)
    bool CheckARM9(uint32_t pc);
    bool CheckARM7(uint32_t pc);
